...
def compute_layer_style_cost(a_S, a_G):
    ...
    J_style_layer = tf.reduce_sum(tf.square(tf.subtract(GS, GG)))/tf.cast((4 *  tf.square(n_H * n_W) * tf.square(n_C)),dtype=tf.float32)
...
def train_step(generated_image):
    with tf.GradientTape() as tape:
        ...
        #(1 line)
        J_style = compute_style_cost(a_G, generated_image)

        #(2 lines)
        # Compute the content cost
        J_content = compute_content_cost(a_G, generated_image)
